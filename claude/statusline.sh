...
# Build context progress bar (15 chars wide) with warning colors
bar_width=15
filled=$((context_percent * bar_width / 100))
empty=$((bar_width - filled))

# Determine bar color based on context usage
...
if [ -d "$current_dir" ]; then
    ...
    if [ "$todo_count" -gt 0 ]; then
        ...
        todo_info=" ${GRAY}|${NC} ${TODO_COLOR}TODO:${todo_count}${NC}"
    fi
fi

# Build context bar display with color
context_info="${BAR_COLOR}${bar}${NC} ${BAR_COLOR}${context_percent}%${NC}"

# Output the status line
echo -e "${BLUE}${dir_name}${NC} ${GRAY}|${NC} ${CYAN}${model_name}${NC} ${GRAY}|${NC} ${context_info}${git_info:+ ${GRAY}|${NC}}${git_info}${todo_info}${cost_info}"